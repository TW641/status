(function() {
  ...
  console.log("🚀 jsDelivr Inline 攔截器：0 毫秒光速啟動！防護網架設完畢！");
  ...
  // ========================================================
  // 🛡️ 核心防護一：底層原型鏈同步攔截 (徹底解決瀏覽器同步載入圖片的 404 洩漏)
  // ========================================================
  ...
  // 全局 Fetch 攔截 (已拔除危險的 CDN Mock，僅保留 GitHub API 保護)
  ...
  window.fetch = async function(...args) {
    ...
    try {
      ...
      // 僅針對 GitHub API 進行錯誤降級， CDN 檔案錯誤直接交還給瀏覽器原生處理，絕不 Mock！
      if (!response.ok && reqUrl.includes('api.github.com')) {
        console.error(`⚠️ GitHub API 回傳錯誤 (${response.status})，自動攔截以維持前端穩定。`);
        return new Response(JSON.stringify([]), { status: 200, headers: { 'Content-Type': 'application/json' } });
      }
      return response;
    } catch (error) {
      console.error('🌐 網路請求例外:', error);
  ...
  const fixHeadingHierarchy = () => {
    ...
    document.querySelectorAll('h4').forEach(h4 => {
      ...
      while (h4.firstChild) { h3.appendChild(h4.firstChild); }
      h4.parentNode.replaceChild(h3, h4);
      hasReplaced = true;
    });
    if (hasReplaced) console.debug("🛡️ A11y 階層修復完成。");
  };
  ...
  // 🛡️ 核心防護二：rAF 渲染防抖 (保留！解決 71ms 強制重排與 Lighthouse 掉分的關鍵)
  scanAndFixDOM();
  let scanFrame;
  ...
  document.addEventListener('click', async (e) => {
    ...
    if (targetLink && targetLink.getAttribute('href') === '#purge-cache') {
      ...
      const purgeRequests = Array.from(currentUrls).map(url => {
        ...
        return fetch(purgeUrl, { mode: 'cors' }).catch(err => { console.error('❌ 清除快取失敗:', err); });
      });
      ...
      if (!reloadUrl.pathname.endsWith('/') && !lastSegment.includes('.')) { reloadUrl.pathname += '/'; }
